...
/**
 * class NextHop stores the ip address and port id of the next router interface packets should be routed to
 */
class NextHop{
	/**
	 * ip address
...
class SimInterface extends Thread{
	...
	/**
	 * denotes whether interface is configured, true if configured and false otherwise
	 */
	 ...
	 *
	 * @param ip address of the router interface
	 */
	 ...
	 *
	 * @param mac address of the router interface
	 */
	 ...
	 */
	byte getMacAddress(){return dll.getMacAddress();}
	
	/**
	 * sets subnet mask of the router interface
	 *
	 * @param subnet mask of the router interface
	 */
	 ...
	 * @param b boolean indicating status of the interface, true if configured and false otehrwise
	 */
	 ...
	 *
	 * @return boolean indicating status of the interface, true if configured and false otehrwise
	 */
	 ...
	 * 
	 * @param s true if interface is up and false otherwise
	 */
	 ...
	 * @param buffer of frames
	 * @param buffer of packets
	 */
	public SimInterface(String dId, int iId, Buffer<ByteArray> mem, Buffer<ByteArray> pbuf){				
		deviceId=dId;
		interfaceId=iId;
		...
		dll=new DataLinkLayer(deviceId, Integer.toString(interfaceId),(byte)0, fromDll, pbuf);
		start();
	}
	public void run(){	
		try{
...
class NetworkAddress
{
	IpAddress ip;
	int mask;
	NetworkAddress(IpAddress ip,int mask)
	{
		this.ip=ip;
		this.mask=mask;
	}
	...
	public boolean equals(Object obj)
	{
		...
		return ip.sameIp(addr.ip) && mask==addr.mask;
	}
	@Override
	public int hashCode()
	{
		return RIPUpdate.byteArrayToInt(ip.getBytes())+mask;
	}
}
...
class SimRouter extends Thread{
	...
	HashMap<NetworkAddress,RoutingTableEntry > routingTable = new HashMap<NetworkAddress,RoutingTableEntry >();
	/**
		...
		while((line=br.readLine())!=null){
			...
			if(tokens[0].compareTo(deviceId)==0){
				...
				if(paramName.compareTo("NUMOFPORTS")==0){
					...
					interfaces=new SimInterface[interfaceCount+1];	//to start index from 1	
					...
					for (int i=1; i<=interfaceCount; i++){
						...
						interfaces[i]=new SimInterface(deviceId, i, rMemory, interfaceBuffers[i]); //each port take instance of the switch and its id			
					}
				}				
				else if(paramName.compareTo("PORTIPMASK")==0){
					int interfaceId=Integer.parseInt(tokens[2]);					
					interfaces[interfaceId].setIpAddress(new IpAddress(tokens[3]));
					interfaces[interfaceId].setSubnetMask(Integer.parseInt(tokens[4]));					
					
					interfaces[interfaceId].setIsConfigured(true);
					System.out.println("Port IP Mask: "+interfaceId+" "+interfaces[interfaceId].getIpAddress().getString()+" "+interfaces[interfaceId].getSubnetMask());										
				}
				else if(paramName.compareTo("PORTMAC")==0){
					int interfaceId=Integer.parseInt(tokens[2]);					
					interfaces[interfaceId].setMacAddress((byte)Integer.parseInt(tokens[3]));
					System.out.println("Port "+interfaceId+" MAC: "+(interfaces[interfaceId].getMacAddress()& 0xFF));											
		...
		while((line=br.readLine())!=null){
			...
			for(int i=1;i<=interfaceCount;i++){
				if(interfaces[i].getIpAddress()!=null){
					if(dst.sameSubnet(interfaces[i].getIpAddress(), interfaces[i].getSubnetMask())){
	 ...
	 * @param status status of the interface, true if interface is on and false otherwise
	 */
	public void setPortStatus(int interfaceId, boolean status){
		...
		else System.out.println("Invalid port number");
	}
	/**
	 ...
	 *
	 * @param p ByteArray representing packets
	 * @param outInterface interface id to which the data should be forwarded to
	 */
				...
				if(interfaces[interfaceId].getPortStatus()==true){
					...
					//2. else if it is data packet, then route
						...
						if (nextHop==null){//unreachable
							...
							Packet rep=new Packet(interfaces[interfaceId].getIpAddress(), p.getSrcIp(), text.getBytes());
						
							nextHop=rProto.getNextHop(p.getSrcIp());
							
							if (nextHop==null)System.out.println("Source Unreachable");
							else {
								...
								else System.out.println("MAC not found in ARP Table; Dropping Packet");
							}
						...
						//2.2 A route found for the packet					
							...
							if((interfaces[nextHop.getInterfaceId()].getIpAddress()).sameIp(nextHop.getIp()))
...
class ConsoleInput extends Thread{
	...
	/**
	 * @param s SimRouter representing the simulated router
	 */
