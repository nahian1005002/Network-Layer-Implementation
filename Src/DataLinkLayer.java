...
class DataLinkLayer {
	 ...
	 *            port id
	 * @param otherId
	 *            port id of the device at the other end
	 * @param mac
	 ...
	 *            buffer of packets
	 */
	DataLinkLayer(String deviceId, String pId, byte mac, Buffer<ByteArray> d,
			Buffer<ByteArray> n) {
		portId = pId;
		dll2nl = d;
		nl2dll = n;
		...
		if (portId != "")
			deviceWithPortId = deviceId + "-" + portId;
		simPhy = new SimPhy(deviceWithPortId, phy2dll, dll2phy);

		// if (srcMac==0) simPhy.setPortStatus(false); //a port will be down
		// without mac address; default could have been used instead;
	 ...
	 * 
	 * @param s
	 *            true if port is up and false otherwise
	 */
	public void setPortStatus(boolean s) {
		simPhy.setPortStatus(s);
	}
	...
	/**
	 * class DllSend receives packets from network layer and sends frame to data
	 * link layer
	 */
		...
		public void run() {
			try {
				while (true) {
					...
					byte[] p = pktWithDstMac.getAt(1,
							pktWithDstMac.getSize() - 1);
					...
					if (portId.compareTo("") == 0)
						...
						System.out.println("Sent Frame Through Port= " + portId
								+ " : " + f.getString() + "\n");
				}
			} catch (Exception e) {
				e.printStackTrace();
			...
			try {
				while (true) {
					...
					if (portId.compareTo("") == 0)
						...
						System.out.println("\tReceived Frame on Port= "
								+ portId + " : " + f.getString() + "\n");

					// check if the frame is valid and if so, send to upper
					// layer
					if (f.getDstMac() == srcMac
							|| f.getDstMac() == BROADCAST_MAC) {
						if (f.hasCheckSumError()) {
							System.out.println("\tChecksum Erron in Frame: "
									+ f.getString() + "\n");
						} else
							sendToNl(f.getPayload());
					} else
						System.out.println("\tMac Mismatch. Dropping Frame: "
								+ f.getString() + "\n");

					/*
					 * synchronized(dll){ dll.receiveHandler(f); }
					 */
				}
			} catch (Exception e) {
			}
