...
//					while(!Res.haveSomethingToRead)
//					{
//						
//					}
//					String input = Res.MSG_TO_SEND;
//					cp.jtDebug.setText(Res.MSG_TO_SEND);
//					Res.haveSomethingToRead = false;
					String[] tokens= input.split(":");			
					if(tokens.length<2){
						System.out.println("Input parameter missing");
						//cp.jtError.setText("Input parameter missing");
						continue;
					}
					...
					if (ipAddr.sameIp(dstIp)){
						...
						//Res.x = "Ping to own interface";
						//cp.jtReceived.setText(Res.x);
						continue;
					}	
					...
					if(dstMac<0) {
						...
						//cp.jtError.setText("MAC Address of Destination not found");
						continue;
					}
